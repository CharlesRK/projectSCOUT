        ...
        while self.running:
            ...
            if is_not_empty:
                ...
                for corner in markerCorners:
                    ...
                    print(f"ARUCO DETECTED: Relative Position = ({relative_cX}, {relative_cY})")

                    # Determine drone movement based on the marker's position
                    move_x = 0
                    move_y = 0
                    sensitivity = 50  # Adjust based on the frame size and drone's responsiveness
                    
                    if abs(relative_cX) > sensitivity:
                        move_y = 0.1 if relative_cX < 0 else -0.1  # Note: Adjusting move_y for horizontal movement due to drone's movement axes
                    
                    if abs(relative_cY) > sensitivity:
                        move_x = -0.1 if relative_cY < 0 else 0.1  # Note: Adjusting move_x for vertical movement, assuming moving "forward" decreases y in the camera's frame of reference

                    # Execute movement command if necessary
                    if move_x != 0 or move_y != 0:
                        print(f"Moving drone to align marker: dx={move_x}, dy={move_y}")
                        #self.drone(moveBy(move_x, move_y, 0, 0) >> FlyingStateChanged(state="hovering", _timeout=5)).wait()
